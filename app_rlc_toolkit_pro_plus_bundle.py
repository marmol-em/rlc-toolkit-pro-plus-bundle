# ==========================================================
# R-L-C TRANSMISSION LINE TOOLKIT — PRO+ (Bundled, Units)
# ----------------------------------------------------------
# Layout: Tabbed (Resistance | Inductance | Capacitance | Summary)
# Accent: Electric Blue
# Features:
#   • Resistance with temperature correction (per km & total)
#   • Inductance & Capacitance (per km & total) with GMR & GMD
#   • Switchable bundled conductors (n = 1..6) + bundle spacing
#   • Global unit selector (SI / US) + optional dual display
#   • Frequency-aware: X_L and B_C (per km), Surge impedance sqrt(L/C)
#   • Beginner-first UI, geometry sketches, trend plots, CSV export
#   • Unique Streamlit keys (no duplicate ID errors)
# ==========================================================
...
.result-title { font-weight: 600; font-size: .95rem; color: #334155; margin-bottom: 6px; }
.result-value { font-weight: 800; font-size: 1.15rem; color: var(--title); }
...
def to_si_radius(val, unit_system):   # radius: SI=mm, US=inch
    return (val/1000.0) if unit_system.startswith("SI") else (val * INCH_TO_M)

def to_si_spacing(val, unit_system):  # spacing/height/coordinates: SI=m, US=ft
    return val if unit_system.startswith("SI") else (val * FT_TO_M)

def to_si_length(val, unit_system):   # line length: SI=km, US=mile
    return (val*1000.0) if unit_system.startswith("SI") else (val * MILE_TO_M)

def to_si_area(val, unit_system):     # metallic area: SI=mm², US=kcmil
...
def rho_corrected_constantC(rho1_ohm_m: float, T1: float, T2: float, C_const: float) -> float:
    """Temperature-corrected resistivity using 'constant C' model."""
    return rho1_ohm_m * (T2 + C_const) / (T1 + C_const)

def gmr_from_radius(radius_m: float, user_gmr_m: float = 0.0) -> float:
    """GMR: user override or 0.7788*r for solid round conductor."""
    return 0.7788 * radius_m if user_gmr_m == 0 else user_gmr_m

def gmd_single(d_m: float) -> float:
    """GMD for single-phase two-wire."""
    return d_m

def gmd_three(xA, yA, xB, yB, xC, yC):
    """GMD for 3φ (transposed average): (Dab*Dbc*Dca)^(1/3). Returns GMD and three spacings."""
...
def bundle_eq_gmr(gmr_single_m: float, bundle_n: int, bundle_spacing_m: float) -> float:
    """Equivalent GMR for symmetric bundle: (GMR_single * d_b^(n-1))^(1/n)."""
    if bundle_n <= 1:
        return gmr_single_m
...
def bundle_eq_radius(radius_m: float, bundle_n: int, bundle_spacing_m: float) -> float:
    """Equivalent radius for capacitance for symmetric bundle: (r * d_b^(n-1))^(1/n)."""
    if bundle_n <= 1:
        return radius_m
...
def C_per_m_single(d_m: float, r_m: float, h_m: float) -> float:
    """Capacitance per meter for single-phase using image method (height optional)."""
...
with tabR:
    ...
    with col1:
        material = st.selectbox("Conductor material", ["Copper", "Aluminum"], key="res_mat")
        # dynamic labels
        ...
        rho1_val = st.number_input("ρ₁ at T₁ (value)", value=1.724e-8 if material == "Copper" else 2.826e-8,
                                   format="%.6e", key="res_rho1")
        rho_unit = st.selectbox("Resistivity unit", RHO_UNIT_OPTS, index=0, key="res_rhounit")
        area_in = st.number_input(AREA_LABEL, value=300.0, min_value=0.1, key="res_area")
        length_in_R = st.number_input(LENGTH_LABEL_R, value=10.0, min_value=0.001, key="res_len")
    with col2:
        T1 = st.number_input("Reference temperature T₁ (°C)", value=20.0, key="res_T1")
        T2 = st.number_input("Operating temperature T₂ (°C)", value=50.0, key="res_T2")
        st.caption(f"Temperature constant θ = **{TEMP_CONST[material]} °C** for {material}")

    # Convert to SI and compute
    area_m2 = to_si_area(area_in, unit_system)
    length_m_R = to_si_length(length_in_R, unit_system)
    rho1_SI = to_si_rho(rho1_val, rho_unit)
    ...
    with c2:
        # Optionally show Ω/mile too
        R_per_m = R_per_km / 1000.0
        R_per_mile = R_per_m * MILE_TO_M
        if unit_system.startswith("US") or show_dual:
            pretty_card("R per km", fmt_dual(f"{R_per_km:.6f}", "Ω/km", f"{R_per_mile:.6f}", "Ω/mile"))
        else:
            pretty_card("R per km", f"{R_per_km:.6f} Ω/km")
    with c3:
        pretty_card("Total R", f"{R_total:.6f} Ω")

    st.markdown('<div class="section-note">R increases with temperature; larger metallic area reduces R.</div>', unsafe_allow_html=True)
...
with tabL:
    ...
    st.markdown("Select system type, geometry, optionally enable bundled conductors, and set frequency to see Xₗ.")

    sys_type_L = st.radio("System", ["Single-phase (two-wire)", "Three-phase (transposed)"], horizontal=True, key="ind_sys")
    ...
    if sys_type_L.startswith("Single"):
        SP_LABEL = "Spacing d (m) between the two conductors" if unit_system.startswith("SI") else "Spacing d (ft) between the two conductors"
        d_in = st.number_input(SP_LABEL, value=2.0, min_value=0.01, key="ind_d")
        d_m = to_si_spacing(d_in, unit_system)
        GMD = gmd_single(d_m)
        Dab = Dbc = Dca = None
        y_for_plot = 10.0  # for simple sketch
    else:
        st.markdown("**Phase coordinates (m or ft):**")
        colA, colB, colC = st.columns(3)
        with colA:
            xA_raw = st.number_input("xA", value=0.0, format="%.3f", key="ind_xA")
            yA_raw = st.number_input("yA", value=20.0, min_value=0.1, format="%.3f", key="ind_yA")
        with colB:
            xB_raw = st.number_input("xB", value=8.0, format="%.3f", key="ind_xB")
            yB_raw = st.number_input("yB", value=20.0, min_value=0.1, format="%.3f", key="ind_yB")
        with colC:
            xC_raw = st.number_input("xC", value=16.0, format="%.3f", key="ind_xC")
            yC_raw = st.number_input("yC", value=20.0, min_value=0.1, format="%.3f", key="ind_yC")

        # Convert to SI if US
        xA, yA = to_si_spacing(xA_raw, unit_system), to_si_spacing(yA_raw, unit_system)
        xB, yB = to_si_spacing(xB_raw, unit_system), to_si_spacing(yB_raw, unit_system)
        xC, yC = to_si_spacing(xC_raw, unit_system), to_si_spacing(yC_raw, unit_system)

        GMD, Dab, Dbc, Dca = gmd_three(xA, yA, xB, yB, xC, yC)
        st.caption(f"Phase spacings (SI): Dab={Dab:.3f} m • Dbc={Dbc:.3f} m • Dca={Dca:.3f} m")
    ...
    with cR1:
        # Dual display for GMR/GMD when desired
        GMR_ft = GMR_used / FT_TO_M
        pretty_card("GMR used",
            fmt_dual(f"{GMR_used:.6f}", "m", f"{GMR_ft:.6f}", "ft") if (unit_system.startswith("US") or show_dual) else f"{GMR_used:.6f} m"
        )
    with cR2:
        GMD_ft = GMD / FT_TO_M
        pretty_card("GMD",
            fmt_dual(f"{GMD:.6f}", "m", f"{GMD_ft:.6f}", "ft") if (unit_system.startswith("US") or show_dual) else f"{GMD:.6f} m"
        )
    with cR3:
    ...
    ax = plt.gca()
    if sys_type_L.startswith("Single"):
        ax.scatter([0, d_m], [y_for_plot, y_for_plot])
        ax.text(0, y_for_plot + 0.6, "Cond 1", ha="center")
        ax.text(d_m, y_for_plot + 0.6, "Cond 2", ha="center")
        ax.hlines(y=y_for_plot, xmin=0, xmax=d_m, linestyles="dashed")
        if bundle_mode:
            rad = db_m / 2.0
            for xc in [0, d_m]:
        ...
        plt.title("Single-phase geometry (SI units)")
    else:
        ...
        if bundle_mode:
            ...
            for xc, yc in [(xA, yA), (xB, yB), (xC, yC)]:
                for k in range(bundle_n):
                    ...
                    ax.scatter(bx, by, s=10)
        plt.title("Three-phase geometry (phase centers & bundles, SI)")
    plt.xlabel("x (m)")
    plt.ylabel("y (m)")
    ...
    plt.title("Inductance vs spacing (bundle setting applied)")
    plt.grid(True, linewidth=0.4)
    st.pyplot(fig2)
...
with tabC:
    ...
    st.markdown("Pick system, geometry, height, optionally enable bundled conductors, and see B_C at frequency.")
    ...
    with c2:
        ...
        height_in = st.number_input(HEIGHT_LABEL, value=12.0, min_value=0.0, key="cap_h")
    ...
    # Bundle for capacitance uses equivalent radius
    ...
    if sys_type_C.startswith("Single"):
        ...
        st.markdown("**Phase coordinates (m or ft):**")
        colA, colB, colC2 = st.columns(3)
        with colA:
            xA_raw = st.number_input("xA", value=0.0, format="%.3f", key="cap_xA")
            yA_raw = st.number_input("yA", value=20.0, min_value=0.1, format="%.3f", key="cap_yA")
        with colB:
            xB_raw = st.number_input("xB", value=8.0, format="%.3f", key="cap_xB")
            yB_raw = st.number_input("yB", value=20.0, min_value=0.1, format="%.3f", key="cap_yB")
        with colC2:
            xC_raw = st.number_input("xC", value=16.0, format="%.3f", key="cap_xC")
            yC_raw = st.number_input("yC", value=20.0, min_value=0.1, format="%.3f", key="cap_yC")
        ...
        st.caption(f"Phase spacings (SI): Dab={Dab_c:.3f} m • Dbc={Dbc_c:.3f} m • Dca={Dca_c:.3f} m")
    ...
    with e1:
        GMD_ft_eff = GMD_eff / FT_TO_M
        pretty_card("Effective GMD (for C)",
            fmt_dual(f"{GMD_eff:.6f}", "m", f"{GMD_ft_eff:.6f}", "ft") if (unit_system.startswith("US") or show_dual) else f"{GMD_eff:.6f} m"
        )
    with e2:
    ...
    if sys_type_C.startswith("Single"):
        ...
        Cv = [C_per_m_single(dC_m, r_used, h) * 1000.0 for h in hs]  # F/km
        figC = plt.figure()
        plt.plot(hs, Cv)
...
with tabS:
    ...
    st.markdown("All computed values (most recent from each tab). Include this table and one screenshot per tab in your report.")
    ...
    st.download_button("📥 Download CSV", df.to_csv(index=False).encode("utf-8"),
                       file_name="RLC_summary.csv", mime="text/csv")

    st.markdown(
        '<div class="section-note">For validation: compare with textbook examples (e.g., transposed 3φ, equal spacing), '
        'and discuss sensitivity shown in the plots. Bundling should reduce X_L and increase charging C.</div>',
        unsafe_allow_html=True
    )
